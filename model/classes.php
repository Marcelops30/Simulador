...
class Atividade_model
{
	...
	public function atualizar( $id ) {
		$sql = "UPDATE $this->table SET  nome_asm = :nome_asm,
			                         tempo_asm = :tempo_asm,
                                                 pontuacao_asm = :pontuacao_asm,
                                                 imagem_asm = :imagem_asm,
                                                 TB_CICL_SIMU_id_csm = :id_csm
                                        WHERE $this->id = :id";
		$stmt = DB::prepare ( $sql );
		$stmt->bindParam ( ':nome_asm', $this->__get('nome_asm'));
		$stmt->bindParam ( 'tempo_asm', $this->__get('tempo_asm'));
		$stmt->bindParam ( 'pontuacao_asm', $this->__get('pontuacao_asm'));
		$stmt->bindParam ( 'imagem_asm', $this->__get('imagem_asm'));
		$stmt->bindParam ( 'id_csm', $this->__get('id_csm'));
                $stmt->bindParam ( ':id', $id, PDO::PARAM_INT );
...
class CicloSimulador_Model {
		...
		public function atualizar($id){
			$sql  = "UPDATE $this->table SET descricao_csm = :descricao_csm,
                                                         imagem_csm = :imagem_csm,
                                                         TB_COMP_CURC_id_ccr = :id_ccr,
                                                         TB_SIMULADOR_id_sml = :id_sml
                                                                     WHERE $this->id = :id";
			$stmt = DB::prepare($sql);
			$stmt->bindParam(':descricao_csm', $this->__get('descricao_csm'));
...
// Classe Realizar Ciclo
// Autor
class RealizarCiclo_model {
		protected $table = 'tb_relz_cicl';
		protected $id = 'id_Rcc';
    ...
    public function adicionar() {
                ...
                $sql = "INSERT INTO tb_relz_cicl (pontuacaoAlcancada_rcc, TB_CICL_SIMU_id_csm, TB_USUARIO_id_usu) 
                VALUES (:pontuacaoAlcancadaRcc, :id_csm, :id_usu)";
                $stmt = DB::prepare ($sql);
                $stmt->bindParam (':pontuacaoAlcancadaRcc', $this->__get('pontuacaoAlcancadaRcc'));
                $stmt->bindParam (':id_csm', $this->id_csm);
                $stmt->bindParam (':id_usu', $this->id_usu);
                return $stmt->execute();
        }

    public function atualizar($id){
                $sql  = "UPDATE $this->table SET pontuacaoAlcancada_rcc = :pontuacaoAlcancadaRcc,
                							     TB_CICL_SIMU_id_csm = :id_csm;
                							     TB_USUARIO_id_usu = :id_usu
                                             WHERE $this->id = :id";
                $stmt = DB::prepare($sql);
                $stmt->bindParam (':pontuacaoAlcancadaRcc', $this->__get('pontuacaoAlcancadaRcc'));
                $stmt->bindParam (':id_csm', $this->__get('id_csm'));
                $stmt->bindParam (':id_usu', $this->__get('id_usu'));
                $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                return $stmt->execute();
		}
