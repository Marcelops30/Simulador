   ...
   public function __construct(){
    ...
    public function listar(){
        ...
        if (empty($username)){ #Verifica se os campos estão preenchidos
            ...
            redirect('usuario/index');
                    ...
                    if ( ($this->usuario->procurarEm('email_usu', $email)) || ($this->usuario->procurarEm('username_usu', $username)) ){
                    setcookie('msg','Email ou Username ja utilizado!'); #Se não tiver, armazena mensagem para mostrar.
                        ...
                        $this->usuario->__set('nome', $nome);
                        $this->usuario->__set('sobrenome', $sobrenome);
                        $this->usuario->__set('email', $email);
                        $this->usuario->__set('username', $username);
                        $this->usuario->__set('senha', $senha);
                        $this->usuario->__set('cpf', $cpf);
                        $this->usuario->__set('perfil', $perfil);
                        $this->usuario->__set('sexo', $sexo);
                        #$perfil->__set('avatar', $descricao);
                        if ($this->usuario->adicionar()){ #Aqui faz o insert e seta um cookie para mostrar depois dependendo da situação (se deu certo ou não)
    ...
    if (empty($username)){ #Verifica se os campos estão preenchidos
        ...
        redirect('usuario/editar/'.$id);
                ...
                if ($this->usuario->atualizar($id)){ #Aqui faz o insert e seta um cookie para mostrar depois, dependendo da situação (se deu certo ou não)
